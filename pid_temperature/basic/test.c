...
float simulate_system(float measured_value, float setpoint, float input) {
    ...
    return (measured_value + (input) * 0.003);
}
...
int main(int argc, char *argv[]) {
	...
	snprintf(fname, sizeof(fname), "output_%s_%s_%s.xlsx", argv[1], argv[2], argv[3]);
    lxw_workbook  *workbook  = workbook_new(fname);
    lxw_worksheet *worksheet = workbook_add_worksheet(workbook, NULL);
	
	worksheet_write_string(worksheet, 0, 0, argv[1], NULL);//kP
	worksheet_write_string(worksheet, 0, 1, argv[2], NULL);//kI
	worksheet_write_string(worksheet, 0, 2, argv[3], NULL);//kD
	
	worksheet_write_string(worksheet, 1, 0, "Set Point", NULL);
	worksheet_write_string(worksheet, 1, 1, "Current Value", NULL);
    ...
    for (int i = 1; i < 5000; ++i) {
        ...
        worksheet_write_number(worksheet, i+1, 0, setpoint, NULL);
        worksheet_write_number(worksheet, i+1, 1, measured_value, NULL);
